...
/** @ingroup algorithms **/
template<typename Range, typename Body>
  void parallel_reduce( const Range& range, Body& body, const static_partitioner& partitioner, const char* file_name, int line_number ) {
  internal::start_reduce<Range,Body,const static_partitioner>::run( range, body, partitioner, file_name, line_number, __builtin_return_address(0) );
}
...
template<typename Range, typename Value, typename RealBody, typename Reduction>
Value parallel_reduce( const Range& range, const Value& identity, const RealBody& real_body, const Reduction& reduction,
                       const static_partitioner& partitioner, const char* file_name, int line_number ) {
    internal::lambda_reduce_body<Range,Value,RealBody,Reduction> body(identity, real_body, reduction);
