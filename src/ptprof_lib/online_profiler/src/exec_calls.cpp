...
extern "C" {
  void TD_Activate(const char* file, int line, int counter_type) {
    ...
    taskProf = new Task_Profiler(file,line);
  }

  void Fini(const char* file, int line)
  {
    taskProf->Fini(file,line);

    //taskGraph->setStepRegion(0, file, line, false);

    //Generate profiles
    taskGraph->FinalizeAndGenerateProfile();
  }
  ...
  __attribute__((noinline)) void __exec_begin__(unsigned long taskId, const char* file, int line){
    ...
    /*PROF CALL*/taskProf->TP_CaptureExecute(get_cur_tid(),file,line);
  }

  __attribute__((noinline)) void __exec_end__(unsigned long taskId, const char* file, int line){
    /*PROF CALL*/taskProf->TP_CaptureReturn(get_cur_tid(),file,line);
